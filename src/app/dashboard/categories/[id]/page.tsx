...
export default function FamilyProfile() {
  ...
  useEffect(() => {
    if (categoriesData) {
      setCategory(categoriesData?.data?.categories?.find((cat) => cat.id === parseInt(id)));
    }
  }, [id, categoriesData]);
          ...
          <div className="flex flex-col items-center gap-2">
            <Popover open={open} onOpenChange={setOpen}>
              <PopoverTrigger asChild>
                ...
                >
                  {(selectedIcon?.icon || category?.icon_url) ? (
                    <div className={`w-20 h-20 rounded-full flex items-center justify-center ${selectedIcon?.color || category?.icon_url}`}>
                      <img src={selectedIcon?.color || category?.icon_url} alt={selectedIcon?.id || category?.id} className="w-6 h-6" />
                    </div>
                  ) : (
